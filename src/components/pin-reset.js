...
export default function PinReset(props) {
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      if (navigator.onLine) {
        ...
        await request
          .post(baseApiUrl + "pin/recover/")
          .type("application/json")
          .send(data)
          .then((response) => {
            Swal.fire({
              title: "Recovered",
              text: "We have successfully recover your pin. you can now proceed",
              icon: "success",
            }).then(() => {
  ...
  const validateForm = () => {
    ...
    if (newPin !== confirmPin) {
      Toast.fire({
        icon: "error",
        title: "New pin has a mismatch with confirm pin",
      });
